...
[![npm version](https://img.shields.io/npm/v/eslint-config.svg)](https://www.npmjs.com/package/eslint-config)
...
```bash
pnpm add -D eslint-config eslint prettier typescript
```
...
```javascript
import nextjsConfig from "eslint-config/nextjs-no-tailwind";
...
```javascript
import fastifyConfig from "eslint-config/fastify";
